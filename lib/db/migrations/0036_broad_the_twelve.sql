...
CREATE TABLE `__new_order_unit` (
	...
	CONSTRAINT "order_unit_single_origin_chk" CHECK(NOT ("__new_order_unit"."order_line_id" IS NOT NULL AND "__new_order_unit"."purchase_order_line_id" IS NOT NULL))
);
--> statement-breakpoint
INSERT INTO `__new_order_unit`("id", "order_line_id", "order_id", "purchase_order_line_id", "purchase_order_id", "serial_number", "supplier_id", "purchase_cost", "purchase_date", "warranty_end", "asset_tag", "kind", "status", "location", "current_request_id", "current_customer_id", "retired_at", "retirement_reason", "notes", "created_at", "updated_at") SELECT "id", "order_line_id", "order_id", "purchase_order_line_id", "purchase_order_id", "serial_number", "supplier_id", "purchase_cost", "purchase_date", "warranty_end", "asset_tag", "kind", "status", "location", "current_request_id", "current_customer_id", "retired_at", "retirement_reason", "notes", "created_at", "updated_at" FROM `order_unit`;--> statement-breakpoint
...
CREATE INDEX `order_unit_kind_idx` ON `order_unit` (`kind`);--> statement-breakpoint
CREATE UNIQUE INDEX `order_unit_serial_idx` ON `order_unit` (lower(trim("serial_number"))) WHERE "order_unit"."serial_number" IS NOT NULL AND trim("order_unit"."serial_number") <> '';--> statement-breakpoint
CREATE INDEX `order_unit_current_customer_idx` ON `order_unit` (`current_customer_id`);--> statement-breakpoint
