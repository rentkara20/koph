"use client"

import { useState, useTransition } from "react"
import { useRouter } from "next/navigation"
import { useTranslations } from "next-intl"
import { toast } from "sonner"
import { Loader2 } from "lucide-react"
import { Button } from "@/components/ui/button"
import { sendSupplierRfqs } from "@/lib/actions/sourcing"
import { translateActionError } from "@/lib/i18n/action-errors"
import type { SourcingRequestItem, Supplier } from "@/lib/db/schema"

// Subset RFQ panel: pick any items × any suppliers → one RFQ per supplier
// carrying the checked items. Sending again to the same supplier is allowed
// (revision/addition) — history stays visible below.
export function SendRfqForm({
  sourcingRequestId,
  items,
  suppliers,
}: {
  sourcingRequestId: string
  items: SourcingRequestItem[]
  suppliers: Supplier[]
}) {
  const t = useTranslations("sourcing")
  const router = useRouter()
  const [selectedItems, setSelectedItems] = useState<string[]>(items.map((i) => i.id))
  const [selectedSuppliers, setSelectedSuppliers] = useState<string[]>([])
  const [error, setError] = useState("")
  const [pending, startTransition] = useTransition()

  function toggle(list: string[], setList: (v: string[]) => void, id: string) {
    setList(list.includes(id) ? list.filter((x) => x !== id) : [...list, id])
  }

  function handleSubmit() {
    setError("")
    startTransition(async () => {
      const result = await sendSupplierRfqs({
        sourcingRequestId,
        supplierIds: selectedSuppliers,
        itemIds: selectedItems,
      })
      if (result.error) {
        const msg = translateActionError(result.error)
        setError(msg)
        toast.error(msg)
        return
      }
      toast.success(t("rfqsSent"))
      setSelectedSuppliers([])
      router.refresh()
    })
  }

  return (
    <div className="space-y-4 rounded-lg border p-3">
      <p className="text-sm font-medium">{t("sendRfq")}</p>

      <div className="space-y-1.5">
        <p className="text-xs font-medium text-muted-foreground">{t("selectItems")}</p>
        {items.map((item) => (
          <label key={item.id} className="flex items-start gap-2 text-xs">
            <input
              type="checkbox"
              className="mt-0.5"
              checked={selectedItems.includes(item.id)}
              onChange={() => toggle(selectedItems, setSelectedItems, item.id)}
            />
            <span>
              <span className="font-medium">{item.quantity}×</span> {item.supplierDescription}
              {item.partNumber && <span className="text-muted-foreground"> · {item.partNumber}</span>}
            </span>
          </label>
        ))}
      </div>

      <div className="space-y-1.5">
        <p className="text-xs font-medium text-muted-foreground">{t("selectSuppliers")}</p>
        <div className="grid gap-1.5 sm:grid-cols-2">
          {suppliers.map((s) => (
            <label key={s.id} className="flex items-center gap-1.5 text-xs">
              <input
                type="checkbox"
                checked={selectedSuppliers.includes(s.id)}
                onChange={() => toggle(selectedSuppliers, setSelectedSuppliers, s.id)}
              />
              {s.name}
            </label>
          ))}
        </div>
      </div>

      {error && <p className="text-sm text-destructive">{error}</p>}
      <Button
        onClick={handleSubmit}
        disabled={pending || selectedSuppliers.length === 0 || selectedItems.length === 0}
        size="sm"
      >
        {pending && <Loader2 className="me-2 size-4 animate-spin" />}
        {t("sendRfq")}
      </Button>
    </div>
  )
}
