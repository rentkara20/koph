...
export default async function SourcingRequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [t, data] = await Promise.all([getTranslations("sourcing"), getSourcingRequest(id)])
  if (!data) notFound()
  const { request, rfqs } = data
    ...
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t(`sourceTypes.${request.sourceType}` as never)}</p>
        </div>
      ...
      {(request.status === "draft" || request.status === "rfq_sent") && rfqSuppliers.length > 0 && (
        <SendRfqForm sourcingRequestId={id} suppliers={rfqSuppliers} />
      )}

      {rfqs.length > 0 && (
        <div className="space-y-3">
          <p className="text-sm font-medium">{t("rfqs")}</p>
          {rfqs.map((rfq) => (
